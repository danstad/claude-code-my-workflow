...
2. **Monotonicity:** $\beta_j(\cdot)$ is strictly increasing, meaning lower-cost firms bid lower
...
The core identification result is due to **Guerre, Perrigne, and Vuong (2000)**, who showed that the equilibrium condition that firm $i$ of type $j$ sets its bid optimally, which directly maps from the **observable bid distribution** to the **unobservable cost distribution**, without requiring any parametric assumption on $F_j(\cdot)$.
...
The left-hand side $b - c$ is the **markup**, i.e., the profit a firm earns per unit if it wins. The right-hand side is an inverse hazard rate of the bid distribution, adjusted for the presence of rivals of each type.
