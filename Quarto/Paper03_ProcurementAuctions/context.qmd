...
def smooth_cdf(x_raw, y_raw, n=500):
    """Interpolate a CDF onto a dense evenly-spaced grid for smooth rendering."""
    # Remove duplicates and ensure monotone x
    order = np.argsort(x_raw)
    x_raw, y_raw = x_raw[order], y_raw[order]
    _, unique_idx = np.unique(x_raw, return_index=True)
    x_raw, y_raw = x_raw[unique_idx], y_raw[unique_idx]
    x_smooth = np.linspace(x_raw.min(), x_raw.max(), n)
    spl = make_interp_spline(x_raw, y_raw, k=3)
    y_smooth = np.clip(spl(x_smooth), 0, 1)
...
for i, col in enumerate(cols):
    for t in [0, 1]:
        ...
        x_s, y_s = smooth_cdf(sub["bid"].values, sub[col].values)
        fig.add_trace(go.Scatter(
            x=x_s,
            y=y_s,
            mode="lines",
