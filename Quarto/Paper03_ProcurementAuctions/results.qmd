...
# Add vertical lines + staggered annotations at means
y_anchors = {0: 0.97, 1: 0.82}   # paper-fraction height: type 0 higher, type 1 lower
for t in [0, 1]:
    ...
    fig.add_annotation(
        x=mean_val, y=y_anchors[t],
        yref="paper",
        text=f"Mean {'T0' if t==0 else 'T1'}: {mean_val:.3f}",
        showarrow=True,
        arrowhead=2, arrowsize=0.8,
        arrowcolor=colors[t],
        ax=28 if t == 0 else -28,
        ay=0,
        font=dict(size=10, color=colors[t]),
        bgcolor="white",
