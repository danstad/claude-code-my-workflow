...
Before describing the model, it helps to understand why simpler approaches fail.

::: {.compare-grid}
::: {.compare-card ols}
#### OLS / Probit
Regress aspiration on a sponsored dummy and controls.

<span class="cross">✗</span> Selection bias: sponsored children are younger, poorer, more Protestant<br>
<span class="cross">✗</span> Compares children who are not comparable<br>
<span class="cross">✗</span> Cannot separate program effect from pre-existing differences
:::
::: {.compare-card iv}
#### Standard IV (LATE)
Use age-at-arrival dummies as instruments for selection.

<span class="check">✓</span> Corrects for observable selection<br>
<span class="cross">✗</span> Assumes the treatment effect is the same for everyone ($\alpha_1 = \alpha_0$)<br>
<span class="cross">✗</span> Only identifies the effect for "compliers" (LATE)<br>
<span class="cross">✗</span> Misses selection on unobserved gains
:::
::: {.compare-card roy}
#### Binary Roy Model
...
:::
$$S^*_i = \gamma_0 + \gamma_1\,\text{Age6}_i + \gamma_2\,\text{Age7}_i + \gamma_3\,\text{Age8}_i + \gamma_4\,\text{AssetIndex}_i + \gamma_5\,\text{Protestant}_i + \gamma_6\,\text{SiteCI}_i - U_{Si}$$
$$S_i = \mathbf{1}[S^*_i > 0]$$
:::
